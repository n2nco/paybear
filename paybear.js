...
export default class Paybear {
   constructor() {
      ...
      if (typeof options.button !== 'undefined') {
         console.log(options.button); //#paybear-ethereum
         var button = document.querySelector(options.button);
         if (!button) {
            throw new Error('Can\'t find ' + options.button);
         }
         var newButton = button.cloneNode(true);
         button.parentNode.replaceChild(newButton, button);
         newButton.addEventListener('click', function (e) {
            e.preventDefault();
            this.paybearInit.call(that);
         });
      }
      else {
         this.paybearInit.call(that);
   ...
   paybearInit() {
       ...
       if (this.arguments) {
           this.options = this.extendDefaults(defaults, this.arguments);
       }
       ...
       this.resizeListener = function () {
           this.resizeFont(that.state.currencies[that.state.selected]['address']);
       };
       ...
       if (options.settingsUrl) {
           if (options.modal) {
            this.initModal.call(that);
           }

           var xhr = new XMLHttpRequest();
           this.beforeCurrenciesSend.call(that);
           xhr.onload = function () {
               if (xhr.status !== 200) {
                  this.handleCurrenciesError.call(that);
               } else {
                  this.handleCurrenciesSuccess.call(that);
   ...
   app() {
       ...
       if (typeof options.currencies === 'string') {
         this.fetchCurrencies.call(that);
           ...
           if (state.currencies.length > 1) {
            this.fillCoins.call(that);
           } else {
               if (state.currencies[state.selected].currencyUrl) {
                  this.currencyUrlXHR.call(that);
               } else {
                  this.paymentStart.call(that);
               }
           }
       } else {
         this.handleCurrenciesError.call(that);
   ...
   fetchCurrencies() {
    ...
    fillCoins() {
       ...
       this.fillCoinsReset.call(that);

       this.bindUnloadHandler.call(that);
       ...
       that.state.currencies.map(function (item, index) {
           ...
           coin.onclick = function (e) {
               ...
               if (item.currencyUrl) {
                   var xhr = new XMLHttpRequest();
                   this.beforeCurrencySend.call(that);
                   xhr.onload = function () {
                       if (xhr.status !== 200) {
                        this.handleCurrencyError.call(that);
                       } else {
                           try {
                              this.handleCurrencySuccess.call(that);
                               var response = JSON.parse(xhr.responseText);
    ...
    fillCoinsReset() {
       ...
       this.unbindUnloadHandler.call(that);
    ...
    paymentStart(hideTopBack) {
       ...
       that.paymentHeaderTitle.textContent = 'Waiting on Payment';
       that.paymentHeaderHelper.innerHTML = 'Rate Locked 1 ' + code + ' : ' + options.fiatSign + this.formatMoney(rate, 2) + ' ' + options.fiatCurrency;
       that.paymentHeaderHelper.removeAttribute('style');

       // timer
       if (options.timer) {
           that.paymentHeaderTimer.textContent = this.formatTimer(options.timer);
           var time = new Date();
           ...
           state.interval = setInterval(function() {
               ...
               if (timer >= 0) {
                   options.timer = timer;
                   that.paymentHeaderTimer.textContent = this.formatTimer(timer);
               } else {
                   clearInterval(state.interval);
                   clearInterval(state.checkStatusInterval);
               }
       ...
       if (selectedCoin.metamask && typeof web3 !== 'undefined' && window.web3.eth.accounts.length) {
           ...
           walletBtn.addEventListener('click', function(e) {
               ...
               window.web3.eth.sendTransaction({
                   ...
                   value: window.web3.toWei(+coinsToPay)
               }, function(err, data) {
                   ...
                   if(err) {
                       ...
                       console.log(data);
                       //success
                       this.paymentConfirming.call(that, 0);
                   }
               });
           });
       ...
       this.resizeFont(selectedCoin.address);
       ...
       copyAddress.addEventListener('click', function () {
           ...
           this.copyToClipboard(selectedCoin.address);
       ...
       copy.addEventListener('click', function () {
           copyAddress.classList.remove('P-btn-block--copied');
           copy.classList.remove('P-btn-block--copied');
           this.copyToClipboard(coinsToPay);
           copy.classList.add('P-btn-block--copied');
       });

       // tabs
       this.paybearTabs.call(that);
               ...
               xhr.onload = function () {
                   if (xhr.status === 200) {
                       ...
                       if (response.success) {
                           ...
                           this.paymentConfirmed.call(that, response.redirect_url, overPaid);
                       } else {
                           if (response.coinsPaid) {
                               if (response.coinsPaid > coinsPaid) {
                                   ...
                                   if (response.coinsPaid < selectedCoin.coinsValue - maxUnderpaymentCrypto) {
                                    this.paymentUnpaid.call(that, diff, response.coinsPaid);
                                   } else {
                           ...
                           if (checkConfirmations) {
                               if ((typeof response.confirmations === 'number' || typeof response.confirmations === 'string') && !Number.isNaN(+response.confirmations)) {
                                 this.paymentConfirming.call(that, response.confirmations);
                               }
    ...
    paymentUnpaid(diff, paid) {
       ...
       this.setInnerHtml(unpaidDue, selectedCoin.coinsValue + ' ' + selectedCoin.code);
       this.setInnerHtml(unpaidPaid, paid + ' ' + selectedCoin.code);
       this.setInnerHtml(unpaidUnderpaid, diff + '&nbsp;' + selectedCoin.code);
       this.setInnerHtml(unpaidPaidFiat, options.fiatSign + (Math.round(selectedCoin.rate * paid * 100) / 100).toFixed(2));
       ...
       if (options.underpaidUrl || options.underpaidText) {
           ...
           unpaidScreenBtn.addEventListener('click', function toStartScreen(e) {
               ...
               this.paymentStart.call(that, true);
    ...
    paymentConfirming(confirmations) {
       var that = this;
       var options = that.options;
       var state = that.state;
   ...
   paymentConfirmed(redirect, overPaid) {
       var that = this;
       var state = that.state;
       var options = that.options;
   ...
   handleCurrenciesError() {
       ...
       if (that.options.modal) {
         this.handleModalOverlay.call(that);
       }
   }
